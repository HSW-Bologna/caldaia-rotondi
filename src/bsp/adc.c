...
uint16_t bsp_adc_get(bsp_adc_t adc) {
    adc_channel_t channels[BSP_ADC_NUM] = { ADC_CHANNEL_5, ADC_CHANNEL_6,
...
void adc_sample_callback(adc_callback_args_t *p_arg) {
    switch (p_arg->event) {
        ...
        case ADC_EVENT_WINDOW_COMPARE_A: {
            ...
            //bsp_timers_prime_adc_interrupt();

            volatile adc_channel_t channel = p_arg->channel;

            volatile uint8_t below = (g_adc0_ctrl.p_reg->ADCMPLR[0] & (1 << channel))
                    == 0;
            // ADC reached 0
            if (below) {
