...
const adc_cfg_t g_adc0_cfg = { .unit = 0, .mode = ADC_MODE_CONTINUOUS_SCAN,
        ...
        .p_callback = adc_sample_callback,
        /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
#endif
        .rs485_setting = { .enable = SCI_UART_RS485_ENABLE, .polarity =
                SCI_UART_RS485_DE_POLARITY_HIGH,
#if 0xFF != 0x01
                .de_control_pin = BSP_IO_PORT_01_PIN_02,
#else
...
const uart_cfg_t g_uart9_cfg = { .channel = 9, .data_bits = UART_DATA_BITS_8,
        .parity = UART_PARITY_OFF, .stop_bits = UART_STOP_BITS_1, .p_callback =
                rs485_callback, .p_context = NULL, .p_extend =
                &g_uart9_cfg_extend,
